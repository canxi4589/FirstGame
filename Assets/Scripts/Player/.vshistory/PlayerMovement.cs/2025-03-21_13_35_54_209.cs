...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] private Sprite playerDialogSprite; // Player's portrait for dialog
    ...
    private GameObject currentInteractable; // Track the interactable object in range
    ...
    public void Interact(InputAction.CallbackContext context)
    {
        ...
        {
            if (currentInteractable.CompareTag("wutt"))
            {
                if (keys > 0)
                {
                    keys--;
                    UpdateKeyUI();
                    LockObject lockObject = currentInteractable.GetComponent<LockObject>();
                    if (lockObject != null)
                    {
                        lockObject.Unlock();
                        if (SoundManager.Instance != null && Time.time >= lastSoundTime + soundDebounceTime)
                        ...
                        // Disable the door's collider if it exists
                        BoxCollider2D doorCollider = currentInteractable.GetComponentInParent<BoxCollider2D>();
                        if (doorCollider != null)
                        {
                            doorCollider.enabled = false;
                            Debug.Log("PlayerMovement: Door collider disabled");
                        }
                    }
                ...
                else
                {
                    ...
                    {
                        DialogManager.Instance.ShowDialog("I need a key to unlock this door!", playerDialogSprite, 2f);
                    }

                    Debug.Log("PlayerMovement: Not enough keys to unlock the door!");
                }
            }
        }
    }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        else if (other.CompareTag("wutt"))
        {
            currentInteractable = other.gameObject;
            Debug.Log("PlayerMovement: Entered range of interactable object: " + other.gameObject.name);
        }
    }

    void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag("wutt") && currentInteractable == other.gameObject)
        {
            currentInteractable = null;
            Debug.Log("PlayerMovement: Exited range of interactable object: " + other.gameObject.name);
        }
    }
