...
public class PlayerMovement : MonoBehaviour
{
    ...
    private float lastTrapHoleDamageTime = -Mathf.Infinity;
    [SerializeField] private float trapHoleDamageCooldown = 1f;
    ...
    private bool isFalling = false; // Track if the fall animation is playing to prevent multiple triggers
    ...
    void Update()
    {
        ...
        // Handle trap hole fall with animation and damage
        if (isInTrapHoleFallZone && !isFalling)
        {
            ...
            if (!isDashInvincible)
            {
                ...
                isFalling = true; // Prevent multiple fall triggers
                animator.SetTrigger("Fall"); // Trigger fall animation
                StartCoroutine(HandleFallDamage());
            }
        }
        if (isInTrapHoleFallZone && !isFalling && Time.time >= lastTrapHoleDamageTime + trapHoleDamageCooldown)
        {
            bool isDashInvincible = isDashing && Time.time <= dashIFrameEndTime;
            if (!isDashInvincible)
            {
    ...
    private System.Collections.IEnumerator HandleFallDamage()
    {
        yield return new WaitForSeconds(animator.GetCurrentAnimatorStateInfo(0).length); // Wait for fall animation to finish
        TakeDamage(1); // Apply 1 damage after the animation
        isFalling = false; // Allow another fall if the player is still in the trap hole
    }
