...
public class WeaponHolder : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Space)) // Change to your fire input
        {
            Fire();
        }
    }
    ...
    void Fire()
    {
        ...
        // Get Rigidbody2D to move the projectile
        Rigidbody2D rb = projectile.GetComponent<Rigidbody2D>();
        if (rb != null)
        {
            rb.velocity = firePoint.right * projectileSpeed; // Moves in the firePoint's direction
        }
    }
}
