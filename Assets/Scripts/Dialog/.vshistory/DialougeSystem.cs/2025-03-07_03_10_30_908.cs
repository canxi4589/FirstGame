...
public class DialogueSystem : MonoBehaviour
{
    ...
    public List<CharacterSprite> characterSpriteList;  // List for easy assignment in Unity
    ...
    void Awake()
    {
        ...
        foreach (var entry in characterSpriteList)
        {
            if (!characterSprites.ContainsKey(entry.characterName))
            {
                characterSprites.Add(entry.characterName, entry.sprite);
            }
        }
    }
    ...
    {
        if (speaker == "Người dẫn truyện")
        {
            characterImage.gameObject.SetActive(false);
        }
        else if (characterSprites.ContainsKey(speaker))
        {
